...
1. **任务指导**：当 Manager 创建风险核查任务后，主动给一线用户发送任务通知，告知需要提交什么材料
2. **材料审核**：一线用户提交材料后，进行双重判断：
   - 判断材料是否符合任务要求（完整？涵盖所有内容？）
   - 判断风险是否真实存在（真实风险？问题不大？无风险？）
3. **进度督促**：提醒一线人员在 feedback_deadline 之前完成反馈
4. **任务完成确认**：当材料审核通过后，主动询问用户是否确认完成
5. **反馈总结生成**：当任务完成时，根据聊天记录和材料内容生成反馈总结（feedback_summary），总结风险核查结果

**重要**：Staff Agent 不是"指导员"教一线人员如何核查风险，而是"传话筒"传达任务要求 + "材料审核员"审核材料和风险 + "督促者"提醒进度 + "总结者"生成核查结论。

**Manager Agent 核心职责**：
1. **风险数据分析**：分析风险数据，识别需要核查的风险点
2. **任务创建与管理**：创建、分配、跟踪任务
3. **对话交互**：与业务负责人进行自然语言对话，理解需求并执行任务
...
- 已下发 → 反馈中：执行人首次回复消息
- 反馈中 → 已完成：执行人确认完成并通过材料审核
- 已下发/反馈中 → 已超时：超过feedback_deadline未完成
- 已下发/反馈中 → 下发失败：任务发送/分配异常时
- 已创建 → 下发失败：任务分配/发送失败
...
- 更新状态说明：已完成（而非"反馈完成"）
- 新增下发失败状态分支
- Staff Agent 新增职责：任务指导、进度督促、任务完成确认
- 新增 Manager Agent 核心职责说明
