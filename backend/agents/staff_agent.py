...
你需要根据一线人员发送的消息，**智能判断**用户是否已经完成核查工作并确认提交。

**判断标准**：
- 如果用户表达了"完成了"、"可以了"、"确认提交"、"已经核实完毕"等类似意图，意味着用户已完成核查
- 如果用户只是在描述核查进展或补充材料，说明尚未完成

**如果用户已确认完成**：
1. 你需要生成一份反馈总结（给业务负责人/Manager 看的汇报）
2. 必须调用 update_task_status 工具更新任务状态

**反馈总结的要求（重要）**：
这份总结是给**业务负责人（Manager）**看的，用于汇报核查结果。总结应该包含：
- **核查结论**：一线人员核查后的结论（如：是否存在风险、风险是否已解除等）
- **处理说明**：一线人员做了哪些核查动作、为什么得出这个结论
- **证明材料**：上传了哪些材料作为证据

在调用 update_task_status 时：
- status: "已完成"
- feedback_summary: 你生成的反馈总结（这段文字是给业务负责人看的）"""
